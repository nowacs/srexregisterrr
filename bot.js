...
client.on("message", message => {
    if(message.content.toLowerCase() == "srex") 
    return message.channel.send(`${message.author}, Srex adamdır.`)
});
...
let rol = member.guild.roles.cache.find(r => r.name === "Cezalı Hesap");
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('794498134507520030');
  }, ms(sürejail));
}
})
...
client.on("messageUpdate", (oldMessage, newMessage) => {
 ...
 const i = db.fetch(`${oldMessage.guild.id}.kufur`)
    if (i) {
        const kufur = ["oç", "amk", "ananı sikiyim", "ananıskm", "piç", "amk", "amsk", "sikim", "sikiyim", "orospu çocuğu", "piç kurusu", "kahpe", "orospu", "mal", "sik", "yarrak", "sex", "amcık", "amık", "yarram", "sikimi ye", "mk", "mq", "aq", "ak", "amq","amguard","seksüel","sekssüel"];
